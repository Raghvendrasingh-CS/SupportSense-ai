...
function scoreRelevance(query, article) {
  ...
  let score = article.relevancescore || article.relevanceScore || 0.5;
  ...
  if (!articleCategoryWords.some(w => q.includes(w))) {
    score = score * 0.5;
  }
...
export async function searchKnowledgeBase(query, limit = 5) {
  ...
  try {
    ...
    if (!hasFabricCredentials()) {
      const result = await db.query('SELECT id, title, category, content, relevance_score as relevancescore FROM knowledge_base');
    ...
    try {
      ...
      const results = rows
        ...
        .sort((a, b) => b.relevanceScore - a.relevanceScore)
        .slice(0, limit);
      return { results, processingTimeMs: measureEnd(startMs), source: 'simulated-sqlite-fallback' };
    } catch (e) {
      return { results: [], processingTimeMs: measureEnd(startMs), source: 'error' };
    }
  }
}
...
export async function getTicketAnalytics() {
  ...
  try {
    ...
    if (!hasFabricCredentials()) {
      ...
      const volResult = await db.query('SELECT date, ticket_count as count FROM ticket_volume_daily ORDER BY date ASC');
      const catResult = await db.query('SELECT category, count, avg_resolution_hours as avgResolutionHours FROM category_breakdown');
      const dailyVolume = volResult.rows || [];
      const breakdown = catResult.rows || [];
    ...
    logError(MODULE, 'getTicketAnalytics failed — fallback', error);
    try {
      const volResult = await db.query('SELECT date, ticket_count as count FROM ticket_volume_daily ORDER BY date ASC');
      const catResult = await db.query('SELECT category, count, avg_resolution_hours as avgResolutionHours FROM category_breakdown');
      return {
        analytics: {
          ticketVolumeTrend: volResult.rows || [],
          categoryBreakdown: catResult.rows || [],
          slaCompliance: 94.2,
          avgFirstResponseMinutes: 12.4,
          avgResolutionHours: 3.8,
      ...
      return {
        analytics: { ticketVolumeTrend: [], categoryBreakdown: [], slaCompliance: 94.2, avgFirstResponseMinutes: 12.4, avgResolutionHours: 3.8 },
        processingTimeMs: measureEnd(startMs),
        source: 'error',
      };
    }
...
export async function logTicketEvent(ticketId, eventType, metadata = {}) {
  ...
  try {
    ...
    logError(MODULE, 'logTicketEvent failed', error);
    return {
      eventId: `sim-event-${Date.now()}`,
      ticketId, eventType, metadata,
      processingTimeMs: measureEnd(startMs),
...
export async function getSimilarTickets(description, limit = 3) {
  ...
  try {
    ...
    if (!hasFabricCredentials()) {
      const result = await db.query('SELECT id, subject, resolution, resolution_time_hours as resolutionTimeHours, similarity, category FROM resolved_tickets');
      const rows = result.rows || [];
      const filtered = rows.filter(r => r.category === detectedCategory);
      const finalResults = filtered.length > 0 ? filtered : rows;
      return {
        similarTickets: finalResults.slice(0, limit),
        processingTimeMs: measureEnd(startMs),
        source: 'simulated-sqlite'
      };
    }
    ...
    logError(MODULE, 'getSimilarTickets failed — fallback', error);
    try {
      const result = await db.query('SELECT id, subject, resolution, resolution_time_hours as resolutionTimeHours, similarity, category FROM resolved_tickets');
      return {
        similarTickets: (result.rows || []).slice(0, limit),
        processingTimeMs: measureEnd(startMs),
        source: 'simulated-sqlite-fallback',
      };
    } catch (e) {
      return { similarTickets: [], processingTimeMs: measureEnd(startMs), source: 'error' };
    }
  }
}
