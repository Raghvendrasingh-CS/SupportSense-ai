...
const DEFAULT_ANALYTICS = {
  ticketVolumeTrend: [
    { date: 'Mon', count: 20 }, { date: 'Tue', count: 35 },
    { date: 'Wed', count: 25 }, { date: 'Thu', count: 45 },
    { date: 'Fri', count: 30 }, { date: 'Sat', count: 15 },
    { date: 'Sun', count: 10 },
  ],
  categoryBreakdown: [
    { category: 'SharePoint', count: 89 },
    { category: 'Exchange', count: 67 },
    { category: 'Teams', count: 54 },
    { category: 'Identity', count: 41 },
    { category: 'Network', count: 33 },
  ],
  slaCompliance: 94,
...
export default function AnalyticsPanel({ analytics, serviceHealth, tickets, memoryStats }) {
  try {
    ...
    const avgConfidence = tickets?.length > 0
      ? Math.round(tickets.reduce((sum, t) => sum + (t.pipeline?.triage?.classification?.categoryConfidence || 0.85), 0) / tickets.length * 100)
      : 87;
    ...
    const dynamicSLA = totalTickets > 0
      ? Math.min(99, Math.round((resolved / totalTickets) * 100))
      : (slaCompliance || 94);
    const dynamicAvgResponse = (avgFirstResponseMinutes || 12.4);
    const dynamicAvgResolution = totalTickets > 0
      ? (() => {
          const processed = tickets.filter(t => t.pipeline?.resolution?.timePrediction?.predictedHours);
          if (processed.length === 0) return (avgResolutionHours || 3.8);
          const avg = processed.reduce((sum, t) => sum + (t.pipeline.resolution.timePrediction.predictedHours || 3.8), 0) / processed.length;
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="border border-[#0078D4]/40 bg-[#0078D4]/10 rounded-lg p-4">
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center py-1 border-b border-[#0078D4]/20">
                ...
                <div className="flex justify-between items-center py-1 border-b border-[#0078D4]/20">
                  <span className="text-xs text-[#94a3b8]">Escalated tickets</span>
                  <span className="text-xs font-mono font-medium text-[#e2e8f0]">{escalated}</span>
                </div>
                <div className="flex justify-between items-center py-1">
                  <span className="text-xs text-[#94a3b8]">Resolved tickets</span>
            ...
            <div className="border border-green-500/40 bg-green-500/10 rounded-lg p-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between items-center py-1">
                  <span className="text-xs text-[#94a3b8]">Sentiment trends</span>
                  <span className="text-xs font-mono font-medium text-[#e2e8f0]">{repeatCustomers}</span>
                </div>
              </div>
            </div>
    ...
    return <div className="text-red-500 text-sm p-4">Failed to load analytics panel</div>;
  }
}
